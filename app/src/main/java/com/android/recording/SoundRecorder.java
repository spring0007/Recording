...
import com.android.recording.util.RecordingFileList;
import com.android.recording.util.SoundPicker;
import com.android.recording.util.StopWatch;
import com.android.recording.util.StorageInfos;
import com.android.recording.util.Utils;
...
/* @} */
    ...
    TextView mOptionsKey = null;
    TextView mBackKey = null;
    TextView mCenterKey = null;
...
//                mMaxFileSize = 1600 * 182 + 37;
        ...
        mdialog = new Builder(this)
                      ...
                      .setCancelable(true)
                      .create();
        mScannerDialog = new Builder(this)
                      .setNegativeButton(getResources().getString(R.string.button_cancel), null)
    ...
    /* SPRD: Add CurrentDate for  bug582705， @{ */
    ...
    private void initService(){
        ...
        if (fromMMS) {
            ...
            if(!mService.getRecroderComeFrom()) {
                ...
                //saveTost();//bug 636880 the record save toase is show two time
                mRecorder.resetSample();
    ...
    protected void onResume() {
        ...
        if(mService == null) {
            ...
            actualPath = mService.getStorePath();
        }
    ...
    private void startRecord(){
        ...
        if (audioManager.isMusicActive()/*isAudioRecording()*/) {
            new Builder(this)
...
//            stopAudioPlayback();
            ...
            if (!StorageInfos.isPathExistAndCanWrite(RecordService.getSavePath())) {
                Toast.makeText(this, R.string.error_path, Toast.LENGTH_SHORT).show();
         ...
         if(mNowState == Recorder.IDLE_STATE) {
             if (!StorageInfos.isPathExistAndCanWrite(RecordService.getSavePath())) {
                 if(!fromMMS){
                     ...
                     mToast = Toast.makeText(this, R.string.error_path, Toast.LENGTH_SHORT);
                     mToast.show();
                     return;
                 }else{
                     changeStorePath();
             ...
             return;
         } else if (mNowState == Recorder.RECORDING_STATE) {
             /*SPRD:add for bug582705 @{*/
                mRecordButton.setImageResource(R.drawable.record);
                                 ...
                                 if (mRecorder == null) {
                                     ...
                                     if (mRecorder.sampleFile() == null) {
                                         ...
                                         if (fromMMS) {
                                             ...
                                             intent = new Intent(SoundRecorder.this,
                                                     RecordingFileList.class);
...
//                 finish();
 ...
 /* @} */
                     ...
                     case Recorder.IDLE_STATE:
                          Intent intent = null;
                                 if(fromMMS){
                                     intent = new Intent(SoundRecorder.this, SoundPicker.class);
                                     startActivityForResult(intent,SOUNDPICKER_RESULT_CODE);
                                 }else if (!mIsRecordStop&&mRecorder!=null&&mRecorder.sampleFile() == null){
                                     intent = new Intent(SoundRecorder.this, RecordingFileList.class);
                                     startActivity(intent);
                                 }
                          return true;
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        Log.d(TAG,"luonkeydown keyCode="+keyCode);
                ...
                case Recorder.IDLE_STATE:
                    ...
                    */
                    if(mRecorder != null && mRecorder.sampleFile() != null) {
                        ...
                        if (fromMMS){//bug 638861 Add the recording attachment to press the return key to return to the HOME interface
                            finish();
                        }else {
                            moveTaskToBack(true);
                        }
        ...
        if(keyCode == KeyEvent.KEYCODE_MENU){
            ...
            if(UNIVERSEUI_SUPPORT){
                 switch (mNowState){
                     ...
                     case Recorder.SUSPENDED_STATE:
                        mRecorder.stop();
                        mRcorderStarus.setText(R.string.new_recordings);
                        mCenterKey.setText(R.string.key_startrecord);
                        mIsRecordStop = true;
                        return true;
                    case Recorder.IDLE_STATE:
                        mIsRecordStop = false;
        ...
        if (keyCode == KeyEvent.KEYCODE_DPAD_CENTER){
            if (((System.currentTimeMillis() - mKeyDownStart) < 500)){
                mKeyDownStart = 0;
                return true;
            }
            switch (mNowState){
                ...
                case Recorder.SUSPENDED_STATE:
                    if (!mNeedRequestPermissions) {
                        doRecord();
                        mRcorderStarus.setText(R.string.status_recording);
                        mCenterKey.setText(getResources().getString(R.string.key_pause)+" ");
                        mOptionsKey.setText(R.string.key_stop);
                        if (Utils.isRtl()){
                            mOptionsKey.setText(R.string.key_back);
                            mBackKey.setText(R.string.key_stop);
                        }
                    }
                    break;
                case Recorder.RECORDING_STATE:
                    doRecord();
                    mCenterKey.setText(R.string.key_continue);
                    mRcorderStarus.setText(R.string.status_pause);
                    break;
            }
                    ...
                    if (action.equals(Intent.ACTION_MEDIA_EJECT)) {
                        ...
                        if(RecordService.getSavePath().startsWith(path)) {
                            ...
                            if(state == Recorder.RECORDING_STATE || state == Recorder.SUSPENDED_STATE || (state == Recorder.IDLE_STATE && mSaveDialog != null && mSaveDialog.isShowing())) {
                                ...
                                Toast.makeText(SoundRecorder.this,R.string.path_miss_nosave, Toast.LENGTH_SHORT).show();
                            }
                        }
                        /* @} */
    ...
    private void updateUi() {
        ...
        switch (mNowState) {
            case Recorder.IDLE_STATE:
                ...
                /*SPRD:add for bug582705 @{*/
                /*if (SIMPLE_MODE) {
                    if (mService == null || mService.getSampleFile() == null
                    ...
                    if (Utils.isRtl()){
                        mOptionsKey.setText(R.string.key_back);
                        mBackKey.setText(R.string.key_option);
                    }else {
                        mOptionsKey.setText(R.string.key_option);
                        mBackKey.setText(R.string.key_back);
                    }
                    mCenterKey.setText(R.string.key_startrecord);
                    mRcorderStarus.setText(R.string.new_recordings);
                    mRecordButton.setImageResource(R.drawable.button_pause);
                } else {*/
            ...
            case Recorder.RECORDING_STATE:
               ...
               /* if (SIMPLE_MODE) {
                    ...
                    mCenterKey.setText(getResources().getString(R.string.key_pause)+" ");
                    mOptionsKey.setText(R.string.key_stop);
                    mRecordButton.setImageResource(R.drawable.button_record);
                    setTitle(res.getString(R.string.record_your_message));
                } else {*/
                    // mRecordButton.setEnabled(false);
                    // mRecordButton.setFocusable(false);
    ...
    public void onError(int error) {
        ...
        if (message != null && mOnErrrDialog == null) {
            /* SPRD: add @{ */
            mOnErrrDialog = new Builder(this)
                .setTitle(R.string.app_name)
                .setMessage(message)
                //.setPositiveButton(R.string.button_ok, null)
    ...
    protected void dialog() {
        Builder builder = new Builder(SoundRecorder.this);
...
/* @} */
    private void changeStorePath(){
        ...
        int item = RecordingFileList.INTERNAL_STORAGE_ITEM;
        String path = "";
        if (StorageInfos.isExternalStorageMounted()) {
            item = RecordingFileList.EXTERNAL_STORAGE_ITEM;
            path = StorageInfos.getExternalStorageDirectory().getAbsolutePath().toString();
        } else {
            item = RecordingFileList.INTERNAL_STORAGE_ITEM;
            path = StorageInfos.getInternalStorageDirectory().getAbsolutePath().toString();
        }
        if(mService != null){
            mService.initRemainingTimeCalculator(path);
        }
        SharedPreferences fileSavePathShare = this.getSharedPreferences(RecordingFileList.SOUNDREOCRD_TYPE_AND_DTA, MODE_PRIVATE);
        SharedPreferences.Editor edit = fileSavePathShare.edit();
        edit.putString(RecordingFileList.SAVE_STORAGE_PATH, path);
        edit.putInt(RecordingFileList.SAVE_STORAGE_PATH_ID, item);
        /* @} */
        edit.commit();
        if(mSaveDialog != null && mSaveDialog.isShowing()){
            mSaveDialog.dismiss();
    ...
    public void addMediaError() {
        addMediaErrorDialog = new Builder(this)
        .setTitle(R.string.app_name)
        .setMessage(R.string.error_mediadb_new_record)
        .setPositiveButton(R.string.button_ok, null)
        .setCancelable(false)
...
//        if (PermissionChecker.checkSelfPermission(Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
