...
public class RecordService extends Service implements Recorder.OnStateChangedListener{
    ...
    protected void startNotification() {
       ...
       /* if(SoundRecorder.UNIVERSEUI_SUPPORT){
        ...
        if(!mFromMms){
            ...
            //SPRD:Bug 628123 com.android.soundrecorder has PendingIntent Risk of misuse
            notice.contentIntent = PendingIntent.getActivity(this,0,new Intent(this,SoundRecorder.class), 0);
        }
        try {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(PAUSE_ACTION.equals(action)){
                if(mCurrentState == Recorder.IDLE_STATE){
                    ...
                    if (audioManager.isSpeakerphoneOn()) {//isAudioRecording()//water:lqq
    ...
    @Override
    public void onStateChanged(Recorder.StateEvent event, Bundle extra) {
    ...
    public String getStorePath() {
        ...
        path = fileSavePathShare.getString(RecordingFileList.SAVE_STORAGE_PATH, "");
        if("".equals(path)) {
            File pathDir = null;
...
//        String title = formatter.format(date);
        ...
        //cv.put(MediaStore.Audio.Media.ALBUM_ARTIST,//water:lqq
        //        res.getString(R.string.audio_db_artist_name));
    ...
    /* SPRD: fix bug 572475 add feature for support OTG. @{ */
    private String createStorePath(String path){
        File pathFile = new File(path + Recorder.DEFAULT_STORE_SUBDIR);
        if (pathFile.isDirectory()) {
            path = pathFile.toString();
        } else if (!pathFile.exists() && pathFile.mkdirs()) {//bug 636458 sdcard recording file is deleted is not created
            path = pathFile.toString();
            initRemainingTimeCalculator(null);
        }
        mSelectPath = path;//bug 632343 Into the tape recorder, set the recording to save path to internal storage, actually stored in the sdcard
        return path;
    }
    /* @} */
}
